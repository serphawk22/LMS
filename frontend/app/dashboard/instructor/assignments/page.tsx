      ...
      <div className="w-full space-y-8">
        <section className="rounded-3xl bg-white p-10 shadow-sm shadow-slate-200/40">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="text-sm text-slate-500">Manage your course assignments and review student submissions.</div>
          </div>
        ...
        <section className="rounded-3xl bg-white p-8 shadow-sm shadow-slate-200/40">
          <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">Create and manage assignments for your courses.</p>
            </div>
            <Link href="/dashboard/instructor/create-assignment" className="inline-flex items-center rounded-full bg-sky-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-sky-700">
              Create new assignment
            </Link>
          </div>
          <div className="mt-2 text-sm text-slate-500">{assignments.length} total</div>

          {loading ? (
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-10 text-slate-500">Loading assignments…</div>
          ) : assignments.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center text-slate-500">No assignments available.</div>
          ) : (
