...
export default function OrganizationAdminRegisterPage() {
    ...
    <main className="flex min-h-[calc(100vh-64px)] items-center justify-center bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 px-4 py-16">
      <div className="w-full max-w-md space-y-8">
        ...
        <div className="text-center">
          ...
          <h1 className="mt-6 text-3xl font-bold tracking-tight text-white">Create Organization Admin account</h1>
          <p className="mt-2 text-sm text-slate-400">Set up a new organization administrator account for the LMS platform</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="rounded-2xl border border-white/[0.08] bg-white/[0.04] p-8 shadow-2xl backdrop-blur-md">
          ...
          <p className="mt-6 text-center text-sm text-slate-400">
            ...
            <Link href="/organization_admin/login" className="font-semibold text-indigo-400 transition hover:text-indigo-300">Sign in</Link>
          </p>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <p className="mt-3 text-sm text-slate-600">Your organization admin account has been created.</p>
            <p className="mt-2 text-sm text-slate-600">Click OK to go to sign in.</p>
            <div className="mt-6 text-right">
              <button
                type="button"
                onClick={() => {
                  setShowSuccessModal(false);
                  router.push('/organization_admin/login');
                }}
