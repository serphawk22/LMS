  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!adminRoles.includes(incomingRole)) {
        setError('Access denied. This sign-in page is for organization administrators only. Use the regular login for student or instructor accounts.');
        setLoading(false);
        return;
      }
    ...
    <main className="flex min-h-[calc(100vh-64px)] items-center justify-center bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 px-4 py-16 relative">
      ...
      <div className="w-full max-w-md space-y-8">
        ...
        <div className="text-center">
          ...
          <h1 className="mt-6 text-3xl font-bold tracking-tight text-white">Organization Admin sign in</h1>
          <p className="mt-2 text-sm text-slate-400">Sign in to your organization administration panel</p>
        </div>
        ...
        <div className="mt-6 text-center text-sm text-slate-400">
          ...
          <button
            ...
            onClick={() => {
              setError('');
              router.push('/organization_admin/register');
            }}
